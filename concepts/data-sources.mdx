...
description: "Every provider behind the numbers, and how fresh each one is."
icon: "database"
---

Every price, ratio, and filing Tradion shows you came from somewhere specific. This page names each source, says how often the number is refreshed, and ends with a list of what Tradion does not have.

<Frame caption="Five sources, each responsible for a different kind of number.">
  <img src="/images/diagrams/data-sources-map.svg" alt="Map showing five data sources and the categories of data each one supplies to Tradion" noZoom />
</Frame>

## In plain English

Tradion has no market data of its own. It buys each kind from a specialist: prices from Alpaca, company fundamentals and earnings data from a licensed financial-data provider, interest rates from FRED, filings from SEC EDGAR, and your own positions from SnapTrade.
...
Alpaca supplies the live and historical prices behind charts, indicators, and the AI Portfolio Analyst.
...
Company fundamentals, the earnings calendar, historical earnings results, company overview data, and screener output come from a licensed financial-data provider. Earnings call transcripts and analyst estimates come from a licensed earnings-data provider. This is what the AI Portfolio Analyst reads when you ask about a company's numbers rather than its chart.

[AI Earnings Research](/research/ai-earnings-research) reads each company's quarterly financials straight from its SEC filings instead, and takes its transcripts and estimates from the financial-data provider. [Where the numbers come from](/research/earnings-evidence) covers how each figure is checked.
...
Tradion does not stream every tick to the AI. An answer uses the data as of the moment you asked, so a price can have moved since.
