...
Chart Analysis reads a picture of a chart and returns a one-word verdict, three price levels, and a written case for the call. Any image works, a TradingView screenshot, a capture from your broker, a chart posted in a Discord.
...
<Frame caption="Analysis history, every chart you have run, with its entry level and risk/reward ratio.">
  <img src="/images/screens/chart-analyzer-history.jpg" alt="Chart Analysis history listing past TSLA and NVDA analyses, each with a summary, entry price, and risk to reward ratio" noZoom />
</Frame>
...
**HOLD means wait.** Not "hold your position". It means a real setup is on the chart and price has not reached the entry yet. Elsewhere in Tradion this verdict is written **WAIT**; the chart card prints HOLD, under a line reading *Default: No action until conditions change*.

**NO TRADE** is a different answer. HOLD is "not yet". NO TRADE is "there is no directional edge here at all", and the card comes back with its entry, stop, target and game plan removed. That is the quickest way to tell the two apart: HOLD still gives you levels, NO TRADE gives you none. [Reading a verdict](/concepts/reading-a-verdict) has the rest.
...
<Note>
  Chart Analysis reads the image and nothing else. There is no follow-up question box, no live price check against your levels, and no strategy code to export. To ask questions about a chart in conversation, use [Lens](/research/lens), which keeps the analysis in context.
</Note>
