...
<Warning>
  **Lens reads Tradion's own chart, not your other tabs.** It cannot capture a TradingView tab, a desktop platform, or anything else on your screen. If the chart you want is elsewhere, screenshot it and use [Chart Analysis](/research/chart-analyzer).
</Warning>
...
**Use Lens** for a listed symbol you want read with real data behind it. **Use [Chart Analysis](/research/chart-analyzer)** for a chart Lens can't draw, a screenshot from a friend, an unsupported platform, an old capture.
...
When a source is unavailable a banner says so: *some sources unavailable*, or *analysis based on chart vision only*. Treat the second as a Chart Analysis result in the Lens panel, see [Data sources](/concepts/data-sources) for who supplies what.
