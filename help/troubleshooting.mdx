  ...
  <Accordion title="I hit a usage limit unexpectedly" icon="gauge-high">
    <Steps>
      <Step title="Check Settings → Monthly Usage first">
        It names which of the four meters is empty and when they reset.
      </Step>
      <Step title="Know what consumes each meter">
        An **earnings message** is spent on every question you send in AI Earnings Research, follow-ups included; **Stop** does not give it back, and a run that fails on Tradion's side does. A **quant session** is spent when you start a new analysis, not per question inside it. An **agent run** is spent per AI agent step per trigger, an automation with two agent steps costs two runs each time it fires. Chart analyses, Lens captures, autopsies, and portfolio syncs cost nothing.
      </Step>
