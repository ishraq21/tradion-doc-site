...
description: "How to read the Runs tab to debug an automation: what fired, when, and why, plus ordered checks for why nothing fired or no notification arrived."
...
While a run is still going the page refreshes itself every ten seconds. For one automation's history rather than all of them, open the automation and click its trigger history. That view paginates in sets of 20 using cursor navigation; per-channel delivery status is not shown at that list level — click into a run to see it.
