...
A run stuck on **Running** for more than 20 minutes is automatically marked Failed. Seeing that repeatedly on one automation means the agents are timing out rather than erroring — usually too many agents on one canvas.
...
```mermaid
flowchart TB
    A["Triggered — timestamp, price, the readings that made it true"] --> B["Agent steps — one card per agent, in execution order"]
...
<Steps>
  <Step title="Is there a run for it?">
    **If a run exists, it did fire** — your problem is delivery, so skip to the next section. If no run exists, carry on.
  </Step>
  ...
  <Step title="Is a Schedule node gating it?">
    A Schedule node placed alongside signals restricts checking to that window. Conditions that go true outside it are not seen.
  </Step>
</Steps>
...
A failing channel is never disabled automatically, so make the Runs tab your first stop when alerts go quiet. Channel-by-channel fixes: [notification channels](/automations/notifications).
