  ...
  sections.forEach(([t, d], i) => {
    ...
    if (i === 3) {
      [['Entry timing', 0.46], ['Position size', 0.3], ['Exit', 0.24]].forEach(([n, v], j) => {
        ...
        s += rect(150, by - 2, 300 * v, 6, { r: 3, fill: [C.danger, C.warn, C.borderStrong][j], stroke: 'none', sw: 0 });
      });
    }
    if (i === 4) {
      ['Entry', 'Exit', 'Risk'].forEach((n, j) => {
        ...
        s += rect(bx + 52, y + 53, 60, 8, { r: 4, fill: C.grid, stroke: 'none', sw: 0 });
        s += rect(bx + 52, y + 53, 60 * [0.72, 0.4, 0.58][j], 8, { r: 4, fill: [C.accent, C.danger, C.warn][j], stroke: 'none', sw: 0 });
      });
    }
...
export function scorecard() {
  ...
  const items = [
    ...
    ['Exit discipline', 0.38, 'Did you leave when your plan said to — in both directions?', C.danger],
    ['Risk management', 0.58, 'Was the position sized so a loss stayed survivable?', C.warn],
  ];
  ...
  items.forEach(([n, v, d, col], i) => {
    ...
    s += `<circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="${C.grid}" stroke-width="10"/>`;
  ...
  s += rect(28, 268, W - 56, 46, { fill: C.panelAlt, r: 8, stroke: C.accentLine, dash: '4 3' });
  s += text(48, 291, 'A profitable trade can score badly, and a losing trade can score well.', { size: 11.5, weight: 600, font: F.head });
  s += text(48, 308, 'The score is about the decision, not the outcome — that is the whole point of it.', { size: 11, fill: C.dim });
  return svg(W, H, s, {
...
export function autopsyLoop() {
  ...
  steps.forEach(([t, d], i) => {
    const x = 28 + i * (bw + gap);
    const hi = i >= 3;
    s += rect(x, 60, bw, 116, { fill: hi ? C.panelAlt : C.panel, r: 8, stroke: hi ? C.accentLine : C.border });
    ...
    if (i < steps.length - 1) s += flowArrow(x + bw + 4, x + bw + gap - 4, 118, { marker: 'ag', stroke: C.accent });
  });
  s += path(`M${28 + 4 * (bw + gap) + bw / 2},180 L${28 + 4 * (bw + gap) + bw / 2},224 L${28 + bw / 2},224 L${28 + bw / 2},182`, { stroke: C.borderStrong, dash: '4 4', marker: 'a' });
...
export function profileAnatomy() {
  ...
  s += `<circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="${C.grid}" stroke-width="9"/>`;
  ...
  [0.9, 0.72, 0.84, 0.6].forEach((w, i) => s += bar(352, 88 + i * 16, 254 * w, { fill: i === 0 ? C.accentLine : C.grid }));
  ...
  [1, 0.7, 0.42].forEach((k) => {
    ...
    s += `<polygon points="${pts}" fill="none" stroke="${C.grid}" stroke-width="1"/>`;
  });
  ...
  s += `<polygon points="${poly}" fill="${C.accentSoft}" stroke="${C.accent}" stroke-width="1.4"/>`;
  ...
  ['Entering before the setup completes', 'Holding losers past the stop', 'Sizing up after a loss', 'Trading the first ten minutes'].forEach((t, i) => {
    ...
    s += rect(400, y + 6, 140, 8, { r: 4, fill: C.grid, stroke: 'none', sw: 0 });
    s += rect(400, y + 6, 140 * [0.88, 0.66, 0.44, 0.3][i], 8, { r: 4, fill: i === 0 ? C.danger : i === 1 ? C.warn : C.borderStrong, stroke: 'none', sw: 0 });
  ...
  for (let d = 0; d < 5; d++) for (let h = 0; h < 8; h++) {
    ...
    s += rect(660 + h * 30, 232 + d * 20, 24, 15, { r: 3, fill: v > 0.75 ? C.accent : v > 0.45 ? C.accentSoft : C.grid, stroke: 'none', sw: 0 });
  }
  ...
  for (let w = 0; w < 42; w++) for (let d = 0; d < 5; d++) {
    ...
    s += rect(46 + w * 20.5, 398 + d * 13, 16, 10, { r: 2, fill: v > 0.8 ? C.accent : v > 0.55 ? C.accentSoft : v > 0.3 ? C.grid : C.panelAlt, stroke: 'none', sw: 0 });
  }
