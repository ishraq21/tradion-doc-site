...
Email **contact@tradionlabs.com** from the address on the account, before the purge date in your confirmation email, and ask for the deletion to be reversed. Do it early rather than on the last day; the purge runs on a timer, not on request.

## What deletion removes

When the purge runs, your profile and everything attached to it is deleted from the database in one pass: trades, autopsies and their scores, behavioural patterns, analyses, Lens analyses, earnings reports, AI Earnings Research threads and your notebook, chat history, playbook rules, trading goals, portfolio data, memory entries, and the search vectors built from them. Anything left over from features Tradion has since retired, such as earlier research sessions or automations, goes too.
...
There is no one-click account export in the app. To get a copy of your data, email contact@tradionlabs.com and request it. You have a right to it in a structured, machine-readable format, and the request needs to be made before the purge date, not after.
